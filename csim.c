...
void init_cache() {                      
    ...
    for (int i = 0; i < S; i++){
        ...
        cache[i]->next = NULL;
        for (int j = 1; j < E; j++){
            adder = malloc(sizeof(cache_line_t));
            if (adder == NULL){
                printf("Fail to allocate\n");
                exit(1);
            }
...
void access_data(mem_addr_t addr) {
    int tbit = addr >> (s+b);
    int sbit = (addr << (64 - s - b)) >> (64 - s);

    cache_line_t* runner = cache[sbit];
    cache_line_t* pre = NULL;
    for (int i = 0; i < E; i++){
        if (runner->valid == '1' && runner->tag == tbit){
            if (i != 0){
                pre->next = runner->next;
                runner->next = cache[sbit];
                cache[sbit] = runner;
            }
            hit_cnt++;
            return;
    ...
    miss_cnt++;
    if (runner->valid == '1') evict_cnt++;
    free(runner);
    runner = malloc(sizeof(cache_line_t));
    if (runner == NULL){
        printf("Fail to allocate\n");
        exit(1);
    }
